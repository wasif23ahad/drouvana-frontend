...
export default function PipelineHealthPage() {
  ...
  const fetchHealthMetrics = async (isRefresh = false) => {
    if (isRefresh) {
      setRefreshing(true);
      toast.info('Running AI pipeline analysis...');
    }
    ...
    try {
      ...
      if (data) {
        ...
        const rejections = apps.filter((a: any) => a.status === 'REJECTED').length;
        ...
        if (isRefresh) toast.success('Pipeline analysis updated');
      }
    } catch (err) {
      toast.error('Failed to load pipeline health analysis');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
