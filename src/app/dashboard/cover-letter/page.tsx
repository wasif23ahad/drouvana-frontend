...
import { Sparkles, Layers, FileText, ChevronRight, Loader2, Copy, CheckCircle2 } from 'lucide-react';
...
export default function CoverLetterPage() {
  ...
  const highlightKeywords = (text: string) => {
    // Dynamic markup injection for core tech terms defined in our guidelines
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center text-xs text-text-muted gap-2 font-jetbrains uppercase tracking-widest mb-2">
            ...
            <span className="text-primary font-bold">Cover Letters</span>
          </div>
          <h1 className="text-3xl font-bold font-hanken">Cover Letters</h1>
          <p className="text-text-sub text-sm">Stream 3 tailored narrative variants dynamically with keyword mapping alignments.</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <Card className="bg-surface/50 border-white/5 backdrop-blur-md rounded-2xl">
            <CardHeader className="pb-3">
              <CardTitle className="text-base font-bold font-hanken text-primary">Target Parameters</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <Label className="text-xs text-text-sub mb-1.5 block">Job Title</Label>
                <Input 
              ...
              <div>
                <Label className="text-xs text-text-sub mb-1.5 block">Company Name</Label>
                <Input 
              ...
              <div>
                <Label className="text-xs text-text-sub mb-1.5 block">Hiring Manager Name (Optional)</Label>
                <Input 
              ...
              <div>
                <Label className="text-xs text-text-sub mb-1.5 block">Narrative Tone</Label>
                <div className="grid grid-cols-3 gap-2 mt-1">
                  {(['analytical', 'enthusiastic', 'executive'] as const).map(t => (
                    <button
                      key={t}
                      onClick={() => setParams({...params, tone: t})}
                      className={`py-2 rounded-lg text-[11px] font-bold capitalize border transition-all ${
              ...
              <div>
                <Label className="text-xs text-text-sub mb-1.5 block">Custom Guidance Instructions</Label>
                <textarea 
                  value={params.customNotes}
                  onChange={e => setParams({...params, customNotes: e.target.value})}
                  rows={3}
                  placeholder="Additional parameters or tech focus..."
              ...
              >
                ...
                {loading ? 'Streaming Sequences...' : 'Generate 3 Variants'}
              </Button>
            </CardContent>
          </Card>
        </div>

        {/* Right Side: Progressive Variant Rendering Viewports */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          {variants.length === 0 && !loading ? (
            <Card className="bg-surface/20 border-white/5 rounded-2xl h-[450px] flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="text-xs text-text-muted max-w-sm">Provide your application specifications on the left to start progressive character streaming generation across 3 distinct architectural templates.</p>
            </Card>
          ) : (
            variants.map((vText, idx) => (
              <Card key={idx} className="bg-surface/50 border-white/5 backdrop-blur-md rounded-2xl overflow-hidden relative">
                <div className="bg-surface-2 px-5 py-3 border-b border-white/5 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold font-jetbrains text-text-main uppercase tracking-wider">
                      Variant {idx + 1} <span className="text-text-muted capitalize font-normal">({idx === 0 ? 'Technical Focus' : idx === 1 ? 'Architectural Balance' : 'Leadership Pitch'})</span>
                    </span>
                  </div>
                  <Button 
                    ...
                    variant="ghost" 
                    size="sm" 
                    className="h-7 text-xs px-2.5 gap-1.5 hover:bg-surface text-text-sub hover:text-text-main rounded-lg"
                  >
                ...
                <CardContent className="p-6">
                  {/* Render with raw injected keyword markup */}
                  <div 
